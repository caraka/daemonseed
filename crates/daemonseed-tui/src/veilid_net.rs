        ...
        tokio::select! {
            cmd = cmd_rx.recv() => {
                ...
                handle_command(
                    ...
                    &confirm_outcome_tx, &mut dm, &mut dm_start,
                ).await;
            }
            ...
            started = wait_dm_start(&mut dm_start), if dm_start.is_some() => {
                ...
                match started {
                    ...
                    Ok(None) => {}
                    Err(e) => daemonseed_veilid_net::vtrace!("tui dm: runner start task failed ({e})"),
                }
            }
    ...
    match event {
        ...
        None => {
            *dm = None;
            ...
            daemonseed_veilid_net::vtrace!("tui dm: runner ended");
        }
    }
}
...
async fn stop_runner(handle: RunnerHandle, evt_tx: &UnboundedSender<NetEvent>) {
    ...
    match outcome {
        ShutdownOutcome::Finished(Err(e)) => {
            daemonseed_veilid_net::vtrace!("tui dm: runner ended with an error ({e})");
        }
        ShutdownOutcome::Panicked => {
            daemonseed_veilid_net::vtrace!("tui dm: runner panicked");
        }
...
async fn finish_dm_for_close(dm_start: &mut Option<DmStart>, evt_tx: &UnboundedSender<NetEvent>) {
    ...
    match ended {
        Some(Ok(_)) => {}
        Some(Err(e)) => daemonseed_veilid_net::vtrace!("tui dm: runner stop task failed ({e})"),
        None => daemonseed_veilid_net::vtrace!(
            "tui close: messaging stop still running at the {cap:?} cap, closing without it"
        ),
    }
}
...
async fn dm_runner_over_veilid(
    ...
    let parts = match net.dm_records_parts().await {
        Ok(parts) => parts,
        Err(e) => {
            daemonseed_veilid_net::vtrace!("tui dm: runner not started ({e})");
            return None;
        }
    };
    let DmSessionKeys {
    ...
    match RunnerParts::<VeilidRecords>::over_veilid(
        ...
        Ok(parts) => {
            daemonseed_veilid_net::vtrace!("tui dm: runner started");
...
async fn handle_command(
    ...
    match cmd {
        ...
        NetCommand::Dm(command) => match dm.as_ref() {
            ...
            None => {
                daemonseed_veilid_net::vtrace!("tui dm: no runner, command refused");
                refuse_dm_command(evt_tx, &command);
            }
        },
        // Stop messaging, as a disconnect does: the runner is stopped off this
        // loop, and its task awaited there before a later connect starts another.
...
async fn run_confirm_download(
    ...
    let build_planned = |source: &[ManifestEntry]| -> Result<Vec<PlannedFile>, ConfirmOutcome> {
        ...
        if flat_dest {
            ...
            // single-depth share would otherwise land loose). DL-ISC-8, matching the
            // GUI's `flat_dest` branch.
            ...
            // The zip below pairs these against `indices` positionally, and a zip
            // TRUNCATES rather than failing — a length divergence would silently
            // drop files from the download. `no_scatter` and `place_at_dest` are
            // both one-out-per-one-in; this pins that so a later edit cannot
            // quietly break it.
            debug_assert_eq!(
                dest_rels.len(),
                indices.len(),
                "one dest_rel per selected file"
            );
            Ok(indices
                .iter()
                .zip(dest_rels)
...
mod tests {
    ...
    /// The actor goes on serving while an old runner ends: a chat command is
    /// answered before the old runner's task has.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn a_chat_command_is_served_while_the_old_runner_is_still_ending() {
        let profile = tempfile::tempdir().expect("tempdir");
        let (evt_tx, mut evt_rx) = unbounded_channel();
        ...
        tokio::time::timeout(
            ...
            handle_command(
                NetCommand::SendChat {
                    circle_id: 0,
                    body: "hello".to_owned(),
                    sender_handle: "me#000000000000".to_owned(),
                },
                &evt_tx,
                &mut net,
                &mut ev_rx,
        ...
        .await
        .expect("the chat command waited on the old runner");

        assert!(
            drain(&mut evt_rx)
                .iter()
                .any(|event| matches!(event, NetEvent::ChatError { .. })),
            "the chat command was not answered"
        );
        assert!(
            dm_start.as_ref().is_some_and(|start| !start.is_finished()),
            "control: the old runner must still be ending when the chat is answered"
        );
        assert!(
            wait_dm_start(&mut dm_start)
                .await
    ...
    /// A command reaches the runner through the session's queue, and the runner's
    /// answer carries the command's token.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn a_command_reaches_the_runner_through_the_session_queue() {
        let profile = tempfile::tempdir().expect("tempdir");
        let (evt_tx, _evt_rx) = unbounded_channel();
        let mut session = DmSession::new(
            empty_runner(
                profile.path(),
                EmptyRecords::new(Duration::ZERO),
                RunnerConfig::default(),
    ...
    async fn graceful_close_stops_a_runner_whose_start_is_still_in_flight() {
        ...
        assert!(
            ...
            "the close acked before the start in flight ended"
        );
        ...
        assert!(ack_rx.try_recv().is_ok(), "the close must still ack");
        assert_eq!(
            Arc::strong_count(&alive),
            1,
