...
- `daemonseed-gui`: operator announcements are kept alive in the DHT — a writer (dev: any client; prod: the operator, gated on the announce owner seed) re-publishes its content-addressed announcement slots every `OPERATOR_KEEPALIVE_INTERVAL` (120s) off the actor loop, so their DHT TTLs stay fresh instead of silently expiring. Announcements were otherwise written only on post, so one not recently re-written aged out of the DHT (vanished across sessions). The mutable MOTD slot is excluded — it has no newer-wins in the fold, so re-publishing a stale MOTD could revert a newer one network-wide; MOTD keep-alive waits for the #136 monotonic version. Keep-alive holds only while a seed-holder is online (#141).
