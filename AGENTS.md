...
**Boundary completion gate:** a PR / batch / refresh is not done until every item above is either updated or explicitly confirmed unaffected. State the reconciliation in the PR body or handoff note (e.g. "doc-sync: CHANGELOG entry + signed tag + ISA Decisions/Verification + README Status line + lama vX (spec-conformant) + manifest current_release/in-progress + N gems captured"). When refreshing context (compaction, handoff, new session), run this ritual **first**, so the next context starts from a true state of the world rather than paying to rediscover it.
