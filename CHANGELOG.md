...
- `daemonseed-veilid-net`: the inbound serve lane is bounded — a full intake queue (`SERVE_QUEUE_CAP`) sheds the incoming fetch request on the producer (a shed serve is one fetcher retry the fetch side already does) instead of growing without limit under a serve-latency spike, and concurrent `app_call_reply` tasks are capped by a semaphore (`MAX_CONCURRENT_SERVE_REPLIES`) acquired before sealing, so a fetch burst can neither spawn unbounded reply tasks nor hold unbounded sealed responses in memory (#125).
...
- `daemonseed-gui` Veilid mode: a public-share fetch that fails on a transport-stage error (dead / rotated route) now RE-RESOLVES — it prunes the stale entry (ISC-S30) and kicks a one-shot lobby resweep (`resweep_rendezvous`) so the freshest advert at the share's stable-id slot (last-writer-wins) is re-pulled, recovering a sharer that merely rotated its route instead of the recipient waiting for the periodic re-sweep; an authoritative withdraw (`NotServed`) still prunes without re-resolving, since the share is genuinely gone. Recipient half of #118, complementing the sender-side stable placement already landed (#118).
- `daemonseed-veilid-net`: a share advertised over a route that died *silently* — one veilid never surfaced in a `RouteChange.dead_routes` — is recovered by a slow-cadence advert watchdog (`AdvertWatchdog`, `ADVERT_WATCHDOG_INTERVAL`) that re-publishes every advert on a timer, the only path independent of the observed-death trigger the relevance filter left as the sole refresh cause. The interval is minutes-scale, two orders of magnitude above the coalesce window, and shares the same in-flight + interval gate as a `RouteChanged` refresh (`spawn_refresh_if_due`), so it cannot recreate the refresh storm (#124).
