...
Current release: **v0.26.0** — alpha (post-MVP). The alpha1 MVP landed at v0.14.0 (the full 4-daemon end-to-end gate, `cargo xtask mvp-gate`, passes 10/10); releases since have added share download, public rooms, the client-identity lifecycle, the multi-circle carousel, the at-rest persistence keystone, the TUI share-management surface, the full TUI share loop (v0.19.0), the finished public share path (v0.20.0), the first desktop GUI with real public-Lobby chat (v0.21.0), GUI circle plumbing (v0.22.0), GUI circle chat (v0.23.0), GUI affordance polish (v0.24.0), and — v0.25.0 — GUI persistent identity: a first-start enrollment wizard + daily-login Unlock so the display handle and joined circles survive a relaunch (the GUI alpha's public + circle chat loop, now with a persistent identity), and — v0.26.0 — the GUI Shares tab: public-share browse, download, and publish (real circle shares remain a subsequent milestone).
