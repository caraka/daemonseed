...
- `daemonseed-core`: verified-resume core — `storage::fetched::StagingArea::{persist_manifest, read_stored_manifest}` store the confirmed manifest in a fetch's staging area (deterministic canonical serialization), `manifest_digest` computes its SHA-384 anchor, `verify_stored_manifest` re-checks the staging copy against the profile-anchored digest fail-closed, and `derive_resume_state` re-derives per-file verified/missing chunk sets from bytes on disk (`docs/design/download-subsystem.md` Part 3).
- `daemonseed-core`: `storage::manifest_digest::ManifestDigestStore` — a profile-local redb store mapping `share_id` to the confirmed manifest's SHA-384 digest, the trusted anchor a resume verifies the staging manifest against (`docs/design/download-subsystem.md` Part 3).
- `daemonseed-gui` + `daemonseed-veilid-net`: verified resume for interrupted downloads — a fresh confirm persists the confirmed manifest + records its profile-anchored SHA-384 digest; a re-initiated fetch verifies the stored manifest and HALTS fail-closed if the anchor is missing/tampered/swapped or the sharer's live manifest differs, then re-derives per-file verified chunks from bytes on disk and re-fetches only the missing ones (`run_download` skips already-verified staged chunks via `PlannedFile.already_verified`) (`docs/design/download-subsystem.md` Part 3; DL-ISC-12/20).
- `daemonseed-tui`: verified-resume parity — the TUI download path now anchors + persists the confirmed manifest on a fresh confirm and re-verifies + resumes an interrupted download exactly as the GUI, fail-closed on a missing/tampered anchor or a changed live manifest (`docs/design/download-subsystem.md` Part 3; DL-ISC-12/20).
