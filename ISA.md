...
- [ ] ISC-S17: *(Withdrawn at the v0.33.0 Veilid cutover — no relay to rate-limit; Veilid governs the transport. Client flood defense was ISC-C26, itself withdrawn 2026-09-07 — see its tombstone.)*
...
- [ ] ISC-A-S12: *(Withdrawn at the v0.33.0 Veilid cutover — no relay to track or profile abuse state. Client flood defense was ISC-C26, itself withdrawn 2026-09-07 — see its tombstone.)*
...
- [ ] ISC-C26: *(Withdrawn 2026-09-07 — the Veilid cutover removed the server-id, the APP_HELLO_ACK and the TLS handshake this criterion described, and the backoff type built for it is deleted. The retry cadences that run carry their own delay ladders; the close-cause vocabulary the TUI renders when one is set is `daemonseed_core::backoff::CloseCause`.)*
...
- [x] ISC-C82: An unlocked identity can be renamed — a new display name re-seals the at-rest blob (under the cached key), so the chosen name persists across the next unlock; a profile created nameless before #65 can likewise be given a name (the recovery path). Invalid names (`is_valid_display_name`: empty, line-break, over-length) are rejected, leaving the prior name intact. The cryptographic identity (the handle hash) is unchanged — only the presented name. The command-palette entry that drives this and the live-wire-handle update are deferred to hands-on testing. (#66)
...
- [x] ISC-A-C34: No duplicate publish (paired with ISC-C69). A second publish of an already-published share must never create a second relay listing. Client primary guard: the published list is keyed by the client-local defined root (`daemonseed_tui::app::PublishedShare`), and `[p]` on a root that is already published — or still queued in-flight — is a status-hint no-op (`[u]` first to re-publish); a failed publish never reaches `published`, so the guard stays open and `[p]` stays retryable. Relay backstop: `SharePublishRegistry::publish` REFUSES (never replaces) a second live listing for the same `(owner, name)` — replace semantics would silently drop a distinct share that merely reuses a display name. The key is per-connection, so two daemons can share a display name. The defined root never rides the wire: `NetEvent::PublishStarted` echoes it client-side only, and the `●` marker binds per defined row by root (never by name join), so a duplicate can never be invisible to its publisher again.
...
- [x] CRSH-ISC-30: A sweep terminates whatever the record does. Every subkey GET is bounded by `SWEEP_GET_TIMEOUT` and `SWEEP_READ_FANOUT` of them run at once, so a sweep of a `dflt(o_cnt)` record spends at most `o_cnt.div_ceil(SWEEP_READ_FANOUT) × SWEEP_GET_TIMEOUT` on its reads however many of them never answer, and an unanswered slot does not withhold the slots behind it. Time queued for a read permit is additional to that figure and is not governed by either constant. A read abandoned at the bound counts in `timed_out` and in `failed`, so a wedged record reaches the CRSH-ISC-2 repair-due rule by the same path an erroring one does. Which subkeys are read does not change: `0..o_cnt`, exactly once each, in index order, whatever the record holds — a read pattern that varied with the contents would be a content oracle to the storage host (WB-0), so the bound and the fan-out apply to empty slots too. A sweep's GET holds its read-pool permit for no longer than `SWEEP_GET_TIMEOUT` — a ceiling on that occupancy only, since the permit is acquired outside the bound and read-lane sites outside a sweep are not governed by it; and stopping a sweep early costs the pool nothing either — the reads still in flight are cancelled and their permits released with them. (#397) (probe: `rendezvous::sweep_abandons_a_get_that_never_answers` and `sweep_reads_fan_out_and_delivers_every_populated_slot`, both paused-time; `a_stopped_sweep_issues_no_more_reads_and_strands_no_permit`.)
...
- **Every DM record is sealed at rest, the receive cursor included.** The cursor was the store's one
  plaintext record, on the reading that a read-through page number is not secret. It is not secret
  and it is still a disclosure: eight clear bytes in a fixed-name file is a monotone proxy for how
  many messages a correspondence has received, readable by anyone holding the disk and no key, in a
  layout whose whole purpose is that a file says nothing about what it contains. The exemption also
  ...
  store's own length check, no new code — and the receiver starts from `START`, which is what it does
  for an absent cursor. The cost accepted is that the cursor now draws on the store key's seal budget;
  the write is conditional on the number actually moving, so the draw is one seal per advance rather
...
`ProjectAnnounceSeedSource::load_checked` and `OperatorCredential::from_seed` take the expected key as
