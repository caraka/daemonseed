...
impl App {
    ...
    /// Whether a circle is joined, so chat can actually transmit (ISC-14).
    /// Used to gate the compose box: with no circle joined, Enter is a no-op
    /// (Item F / ISC-C48 / A-C26) — no local echo, nothing transmitted.
    pub fn can_chat(&self) -> bool {
        matches!(self.circle_status, CircleStatus::Joined)
    }
    ...
    ///
    /// With no circle joined (Item F / ISC-C48 / A-C26), Enter is a strict
    /// no-op: nothing is appended to the local transcript and nothing is
    /// transmitted. A false local echo would lie to the user that a message
    /// "sent" when it never left the machine — verified 2026-06-04 against a
    /// peer that received nothing. Typing into the box pre-join is still
    /// allowed (so the user can draft), but it cannot masquerade as posting.
    fn on_key_chat(&mut self, key: KeyEvent) {
        match key.code {
            ...
            KeyCode::Enter if !self.compose.is_empty() => {
                if !self.can_chat() {
                    // No circle joined → no-op. Surface the requirement instead
                    // of faking a send. The compose buffer is left intact so the
                    // user's draft survives until they join a circle.
                    self.status = Some("join a circle to chat".to_owned());
                    return;
                }
...
mod tests {
    ...
    fn composing_and_sending_queues_chat_and_local_echoes() {
        let mut app = drive_to_main();
        app.on_net_event(NetEvent::CircleJoined); // Item F: chat needs a circle
    ...
    fn enter_with_no_circle_does_not_echo_or_transmit() {
        ...
        assert_eq!(app.status(), Some("join a circle to chat"));
    }
    ...
    fn chat_empty_state_states_circle_requirement() {
        ...
        assert!(
            ...
            "circle requirement stated when no circle joined (ISC-C48)"
        );
        app.on_net_event(NetEvent::CircleJoined);
        let text = render_text(&app, 100, 24);
        assert!(
