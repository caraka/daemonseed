...
| **In-progress / next milestone / state-of-world** | **maintainer-held, out of tree** — intentionally not in this repo | the repo carries no "in-progress / pending / planned" status |
...
4. **`lama.yaml` (root) + `docs/llm-api-manifest/*-api.yaml`** — bump the `version` field to the release; conform both to the LAMA spec, `SPEC.md` at <https://github.com/lamaspec/lama> (root stays a concise capabilities + manifest pointer, **no** milestone/coverage/status); update the per-crate manifests for any crate whose surface the batch changed.
