...
- `daemonseed-{core,gui,tui}`: the open lobby (and circles) order + read-high-water on a **clamped** sender timestamp, not the raw untrusted one — closing the forgeable-ordering vulnerability (#131/#126). `daemonseed_core::transcript::clamp_order_ms` bounds an advisory `sent_unix_ms` to `[now − 24h, now + 120s]` for ordering and the high-water; real backlog (in-window) is returned unchanged and orders by its exact time, while a forged extreme is pulled to a window edge — so an open-room peer can no longer pin the transcript (`i64::MIN`/`i64::MAX`), bury a message far in the past, or suppress unreads with a far-future stamp. De-dup still keys on the original `sent_unix_ms` (stable across DHT re-sweeps), and the read high-water never advances past `now`. Auto-closes #131 and #126 at the v0.33.0 cutover.
