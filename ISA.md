...
- [ ] ISC-C80: *(Withdrawn 2026-09-08 — it described a reconnect driven by a capped exponential backoff timer, and the `Backoff` type that was the timer is deleted and no reconnect path replaced it. The retry cadences that run carry their own delay ladders. ID reserved, never reused.)*
