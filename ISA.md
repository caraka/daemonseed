...
- [ ] ISC-C63: A fully-verified share fetch (ISC-S28 / ISC-A-S20) persists its content to disk under the profile root as an *explicit download*: each verified file's plaintext bytes are written to a content-addressed store (`<profile-root>/fetched/cas`, the `FileChunkStore` addressed by `SHA-384(file-bytes)`) and a manifest record (`share_id → name → [(rel_path, chunk_addr, size)]`) is written to `<profile-root>/fetched/shares.idx`. The download survives the fetch overlay closing and a process restart. This is a deliberately larger at-rest surface than the rest of the client (ISC-A-C1) — downloaded files are user-chosen artifacts, not message/post/session history, so the no-client-history invariant holds; the surface is the R-PANIC erasure target. Content and manifest are plaintext at the MVP (a future at-rest folder-encryption feature wraps the whole `fetched/` tree, additive).
- [ ] ISC-C64: The client surfaces a Fetched-downloads browse view listing every persisted download (ISC-C63) — name, share id, file count, and total size, with the selected download's files (rel_path + size) shown — refreshed from disk when the view opens. The list reflects exactly what is recorded on disk.
- [ ] ISC-C65: From the browse view the user extracts a selected download's files into a chosen destination directory: each file's bytes are reconstructed from the content-addressed store by manifest address and written under `dest`, mirroring the share's `rel_path` tree, byte-for-byte.
...
- [ ] ISC-A-C32: Extracting a fetched download (ISC-C65) must never write a file outside the chosen destination directory. Every manifest `rel_path` is validated to be strictly relative with only normal components — an absolute path, a `..` component, or a root/prefix component is refused — so a hostile sharer's manifest cannot escape `dest` via path traversal.
