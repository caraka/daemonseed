...
- [ ] ISC-C59: The client supports simultaneous membership in multiple circles within a session. Each joined circle is held independently with its own `cot_key` (ISC-C8), rendezvous address `SHA-384(cot_key ‖ server_id)` (ISC-S20), and live `CircleOfTrust.Subscribe` stream. Joining a new circle from entropy (ISC-C8/C9) **adds** to the membership set; it never evicts a currently-joined circle. The membership set is session-scoped — not persisted (consistent with ISC-A-C2: circle entropy is not mnemonic-derivable and is re-entered each session) — but is held in a structure shaped for later serialization into the at-rest blob (ISC-C3), so adding circle-membership persistence is an additive change, not a refactor.
