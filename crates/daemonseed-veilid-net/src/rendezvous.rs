...
/// different shapes — the chat/discovery rendezvous record is `dflt(64)`, while a
/// direct-messaging record is addressed under the subkey count the flows supply —
/// so the engine takes the shape as a parameter rather than baking one in.
///
/// **A shape is derived from the same slot count its address derivation uses,
/// never hand-typed at a call site.** `o_cnt` is part of the address, so a shape
/// that disagreed with the deriving code would open a *different, perfectly valid
/// record* — no error anywhere, and two participants who simply never see each
/// other's writes.
///
...
/// identity and the caches keyed on it.
...
/// catch. The pairing is not reachable today, which is precisely why it is worth
/// closing now, while it is still theoretical.
///
/// The owner half is the **public** key, never the seed it was derived from.
/// An owner seed can be a secret, and under Veilid a derivable owner seed **is**
/// write access to its record. Copying one into a process-lifetime,
/// `Debug`-printable map key would keep that capability in memory for the session,
/// which is exactly what `redacted_secret_newtype::as_bytes` forbids. The public key removes the
...
/// being — a rendezvous, or a record we are about to write. It is wrong for a
/// **read**: reading through `open_or_create` *materializes* a record that does not
/// exist yet — one create plus two un-gated opens (~6–10 s each) spent to
/// manufacture an empty record on the network — and "no such record" and "empty
/// record" collapse into the same `Ok`, because the only way to reach the empty
/// case is to have just created the record.
///
/// **`Ok(None)` is an answer, not a failure.** An absent record is an ordinary
/// state for a read, and is deliberately distinct from `Err`, a transport failure.
///
...
///
/// So a cached handle is held for the session and stays valid.
...
/// `Ok(None)` returns without touching the map. Caching it would turn "this record is
/// not written *yet*" into "this record does not exist" for the remainder of the
/// session: the correspondent writes the record, every later read still answers from
/// the cached absence, and that half of the conversation goes silently dead with no
/// error on any surface. A record that is absent now can be created by the other
/// party at any moment, so absence is a fact about an instant and not about the
/// record.
///
/// The cost of not caching is one open per read of a still-unwritten record.
...
/// that caller ignores it. A caller whose slots carry meaning needs it, and the index is
/// the sweep loop's own variable, so handing it over costs nothing.
...
mod tests {
    ...
    fn write_guard_admits_up_to_the_full_cap_on_a_sparse_shape() {
        let sparse = RecordShape::new(32); // dflt(32) → 32768
        assert!(check_write_cap(32768, sparse).is_ok());
        assert!(check_write_cap(32769, sparse).is_err());
        // A value well inside the cap is admitted.
        assert!(check_write_cap(23 * 1024, sparse).is_ok());
    }
    ...
    fn cached_record_id_separates_the_same_owner_under_different_shapes() {
        ...
        let as_sparse = cached_record_id(&owner.key(), RecordShape::new(32));
        assert_ne!(
            as_rendezvous, as_sparse,
    ...
    /// lock, the way a record read takes it, and a lock this side can take
    ...
    fn every_record_open_goes_through_the_bounded_opener() {
        ...
        // Other modules of this crate take un-gated permits of their own — the
        // subscribe and repair watches — and a record opened
