> **⚠️ SUPERSEDED & ARCHIVED (2026-07-07).** This is the **relay-era** presence design (2026-06-25),
> written before the Veilid Phase 3/4 re-model. It is kept for rationale/history only. The single
> **active** design-of-record is [`docs/design/phase4-veilid-presence-announcements.md`](../phase4-veilid-presence-announcements.md),
> which re-grounds these invariants on the Veilid DHT rendezvous engine (separate presence record via
> `current_state_subkey`; cadence above the ~14.7 s watch floor; the member heartbeat is the only reap
> plane). Do not build from this doc.
