...
- `daemonseed-gui`: the post-connect warmup re-sweep is stepped and priority-ordered — the single delayed re-sweep at +25s is replaced by `WARMUP_RESWEEP_SCHEDULE` (force-refresh re-sweeps at +12/25/45/75s from connect), re-sweeping the content records in priority order (operator MOTD/announce → lobby chat → circle chats, via the unit-tested `warmup_resweep_seeds`) so DHT-converged content surfaces sooner during cold-start than the passive watch alone. Presence records are excluded (they self-heal via the heartbeat cycle); re-swept already-seen items are deduped downstream. Bounded rounds cap the DHT-get load; felt-tunable (#140).
...
- `daemonseed-gui`: operator MOTD + announcements are kept alive in the DHT — a writer (dev: any client; prod: the operator, gated on the announce owner seed) re-publishes its known operator content every `OPERATOR_KEEPALIVE_INTERVAL` (120s) off the actor loop, so the values' DHT TTLs stay fresh instead of silently expiring. Operator content was otherwise written only on post, so an announcement not recently re-written aged out of the DHT (vanished across sessions). Keep-alive holds only while a seed-holder is online (#141).
