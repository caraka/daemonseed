...
- **D-3.5 — anti-dox serve.** Sharer `new_private_route` → publish the route blob (hides the host's node/IP); fetcher addresses `Target::RouteId` from its default safe context (its own safety route hides it; `app_call` replies return along the call path — no separate fetcher blob). Route-blob delivery = an additive `ShareAnnouncement.route_blob` field (MINOR), riding the existing re-announce/roll-call, refreshed on `RouteChanged`. `map_update` starts handling `AppCall`. Slice 2.
